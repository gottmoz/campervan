...
export function FuturisticGauge({ label, value, min = 0, max = 100, unit = "", marks = [], redFrom, accent = "#00d5ff", stale = false, formatter }) {
  ...
  return (
    <div className={`relative h-full min-h-[300px] overflow-hidden border bg-slate-950/75 shadow-2xl backdrop-blur-xl [clip-path:polygon(7%_0,93%_0,100%_9%,100%_91%,93%_100%,7%_100%,0_91%,0_9%)] ${stale ? "border-amber-300/35 shadow-amber-500/10" : "border-cyan-300/40 shadow-cyan-500/20"}`}>
      ...
      <svg viewBox="0 0 360 360" className="absolute inset-0 h-full w-full">
        ...
        {marks.map((mark) => {
          ...
          const markAngle = (-135 + markPercent * 270) * Math.PI / 180;
          ...
          const lx = 180 + Math.cos(markAngle) * 110;
          const ly = 180 + Math.sin(markAngle) * 110;
        ...
        />
        <circle cx="180" cy="180" r="18" fill="#020617" stroke={accent} strokeWidth="2" />
      </svg>
      <div className="absolute inset-x-0 bottom-14 text-center">
        <div className={`text-6xl font-black tracking-tight ${hasValue ? "text-white" : "text-slate-500"}`}>{display}</div>
        <div className="mt-1 text-lg font-bold text-slate-200">{unit}</div>
      </div>
      <div className="absolute inset-x-0 top-[43%] text-center">
        <div className="text-2xl font-black tracking-[0.12em] text-cyan-200">{label}</div>
      </div>
    </div>
  );
}

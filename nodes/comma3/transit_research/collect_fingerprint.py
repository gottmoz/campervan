...
def main() -> None:
    ...
    output = Path(args.output) if args.output else output_dir / f"fingerprint_{int(time.time())}.json"
    payload = {
        ...
        "source": "dry_run",
        "fingerprint": {},
        "notes": "Placeholder until comma/panda read-only CAN capture is attached.",
